...
df_average = (
    df_stats_long.groupby(["variable"])
    .agg(
...
layout = html.Div(
    [
        ...
        dcc.Dropdown(id="user_route", options=df_stats["rt"].unique(), value="171"),
...
def compare_route_with_average(user_route):
    ...
    df_indicators.loc[df_indicators["variable"] == "n_delayed", "variable"] = (
        "Total delayed trips in a week"
    )
    df_indicators.loc[df_indicators["variable"] == "percentage_delayed", "variable"] = (
        "% trips delayed"
    )
    df_indicators.loc[df_indicators["variable"] == "max_delayed_time", "variable"] = (
        "Highest waiting time registered (in minutes)"
    )
    ...
    fig_compare = px.bar(
        ...
        color_discrete_map={"Average": "#8F3931", user_route: "#FFA319"},
    )
    fig_compare.update_yaxes(matches=None)
    fig_compare.update_yaxes(showticklabels=True)
    fig_compare.for_each_annotation(
        lambda a: a.update(text=a.text.replace("variable=", ""))
    )
